...
# Modelo pequeño y barato solo para clasificar el tema de la pregunta (analítica).
...
# ---------------------------------------------------------------------------
# Clasificación de temas (solo para analítica)
# Se clasifica la pregunta en UNA categoría fija. No se guarda ni se registra
# el texto de la pregunta: solo la categoría viaja a la página y a GA4.
# ---------------------------------------------------------------------------
...
def classify_topic(user_texts):
    ...
    try:
        ...
        logger.warning("No se pudo clasificar el tema (se usa 'Otro').")
        return "Otro"
...
def chat():
    ...
    # Clasificación del tema en paralelo a la respuesta principal (no suma tiempo de espera).
    # Se usan las 2 últimas preguntas del usuario para entender seguimientos cortos
    # como "¿y si además tiene fiebre?".
    ultimas_preguntas = [m["content"] for m in cleaned if m["role"] == "user"][-2:]
    ...
    try:
        ...
        tema = "Otro"
    logger.info("consulta_respondida tema=%s", tema)  # solo la categoría, nunca el texto
